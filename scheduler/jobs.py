...
from scheduler.logic import (
    ...
    broadcast_evening, broadcast_night,
    broadcast_weekly, check_and_send_reminders,
...
from scheduler.personal_insights import broadcast_personal_insights  # Daily: персональные корреляции
from config import (
    ...
    NUTRITION_ANALYSIS_TIME, PERSONAL_INSIGHTS_TIME,
)
...
def setup_scheduler(scheduler: AsyncIOScheduler, bot: Bot) -> None:
    ...
    # ── Ночь (23:00) ─────────────────────────────────────────────────────────
    nch, ncm = _parse_time(SCHEDULE_NIGHT_CHECKIN)
    scheduler.add_job(
        broadcast_night, "cron",
        hour=nch, minute=ncm,
        args=[bot],
        id="night_checkin",
        replace_existing=True,
    )
    logger.info(f"Night checkin scheduled: {SCHEDULE_NIGHT_CHECKIN}")
    ...
    # ── Мезоцикл — воскресенье 21:30 (после weekly report, интервал 30 мин) ──
    scheduler.add_job(
    ...
    logger.info("Mesocycle advance scheduled: Sunday 21:30")

    # ── L4 Intelligence — воскресенье 22:15 (после cleanup 22:00) ────────────
    # Фаза 16.3: передаём bot чтобы дайджест отправлялся пользователю в чат
    # Сдвинуто с 21:30 на 22:15 чтобы не конкурировать с weekly+mesocycle в воскресенье
    scheduler.add_job(
    ...
    logger.info("L4 Intelligence update scheduled: Sunday 22:15 (with digest delivery)")
    ...
    # ── Персональные корреляции — воскресенье 20:45 (перед weekly report) ────
    pih, pim = _parse_time(PERSONAL_INSIGHTS_TIME)
    scheduler.add_job(
        broadcast_personal_insights, "cron",
        day_of_week="sun",
        hour=pih, minute=pim,
        args=[bot],
    ...
    logger.info(f"Personal insights scheduled: Sunday {PERSONAL_INSIGHTS_TIME}")
