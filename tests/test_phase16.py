...
async def test_tool_executor_unknown_tool():
    ...
    result = await execute_tool(tg_id=1, tool_name="nonexistent_tool_xyz", tool_input={})
...
@pytest.mark.asyncio
async def test_tool_executor_save_workout_missing_fields(patched_db):
    """save_workout без обязательных полей возвращает error (валидация входных данных)."""
    from ai.tool_executor import execute_tool

    result = await execute_tool(tg_id=1, tool_name="save_workout", tool_input={})
