"""
tests/test_parsers.py — Тесты для активно используемых парсеров.

Удалены тесты мёртвых функций (parse_workout_from_message,
parse_metrics_from_message, is_*_report, parse_nutrition_from_message) —
все они заменены Tool Use / Claude API и больше не вызываются в продакшне.

Оставлены тесты живых функций:
  - parse_exercises_from_message() → db/writer.py
  - _parse_meal_args()             → bot/commands.py
"""
import pytest
