...
# ── Cleanup фантомного event loop от pytest-asyncio ──────────────────────────
# pytest-asyncio при инициализации создаёт event loop через get_event_loop(),
# но не закрывает его. При gc.collect() Python выбрасывает ResourceWarning
# на незакрытый loop и его internal self-pipe сокеты (fd пара).
# Эта фикстура закрывает «бесхозный» loop после завершения всех тестов.
@pytest.fixture(autouse=True, scope="session")
def _cleanup_stale_event_loop():
    yield
    try:
        ...
        pass  # loop уже закрыт или не существует — норма
